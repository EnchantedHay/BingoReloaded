...
public class BingoOptions {
	...
	// General
	// The persistent lobby world. Players wait here between rounds while the dedicated game world is
	// regenerated. The plugin does not create this world, so it must already exist.
	...
	// World Reset (MineHunt-style). The bingo game always runs in a dedicated world that is fully
	// regenerated each round (a background pre-generated _next world that gets swapped in), with the
	// configured defaultWorldName acting as the persistent lobby. Players are scattered safely around
	// the fresh world spawn instead of being teleported to far-away coordinates inside a reused world.
	public static final ConfigurationOption<String> WORLD_RESET_GAME_WORLD_NAME = new StringOption("worldReset.gameWorldName")
			.withEditUpdate(ConfigurationOption.EditUpdateTime.AFTER_SERVER_RESTART);
	public static final ConfigurationOption<Integer> WORLD_RESET_PRELOAD_RADIUS = new IntegerOption("worldReset.preloadRadius").withMin(0)
			.withEditUpdate(ConfigurationOption.EditUpdateTime.AFTER_GAME);
	public static final ConfigurationOption<Boolean> WORLD_RESET_RANDOM_SEED = new BooleanOption("worldReset.randomSeedEachRound")
			.withEditUpdate(ConfigurationOption.EditUpdateTime.AFTER_GAME);
	public static final ConfigurationOption<Integer> WORLD_RESET_SCATTER_RADIUS = new IntegerOption("worldReset.scatterRadius").withMin(0)
			.withEditUpdate(ConfigurationOption.EditUpdateTime.AFTER_GAME);
	public static final ConfigurationOption<Integer> WORLD_RESET_SCATTER_JITTER = new IntegerOption("worldReset.scatterJitter").withMin(0)
			.withEditUpdate(ConfigurationOption.EditUpdateTime.AFTER_GAME);
	public static final ConfigurationOption<Integer> WORLD_RESET_SCATTER_MAX_TRIES = new IntegerOption("worldReset.scatterMaxTries").withMin(1)
			.withEditUpdate(ConfigurationOption.EditUpdateTime.AFTER_GAME);
}

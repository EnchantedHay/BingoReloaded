...
import top.chancelethay.bingo.lib.util.ExtraMath;
...
public class BingoCardMapRenderer extends MapRenderer
{
    ...
    public void render(@NotNull MapView mapView, @NotNull MapCanvas mapCanvas, @NotNull Player player) {
		String currentState = createStateKey();
	...
	private String createStateKey() {
		StringBuilder builder = new StringBuilder(card.getTasks().size() * 32);
		for (GameTask task : card.getTasks()) {
			builder.append(task.data.getType().id)
					.append(':')
					.append(task.data.getRequiredAmount())
					.append(':')
					.append(task.data.getDisplayMaterial(CardDisplayInfo.DUMMY_DISPLAY_INFO).key().asString())
					.append(':')
					.append(task.isVoided());

			task.getCompletedByTeam().ifPresent(completedByTeam -> builder.append(':')
					.append(completedByTeam.getIdentifier())
					.append(':')
					.append(completedByTeam.getColor().value()));

package top.chancelethay.bingo.lib.display;

import top.chancelethay.bingo.data.ScoreboardData;
...
public class ScoreboardDisplay implements SharedDisplay {
	...
	public void update(InfoMenu info) {
		...
		for (UUID id: players) {
			...
			PlayerHandle player = new PlayerHandle(bukkitPlayer);
	...
	public void removePlayer(PlayerHandle player) {
		this.sidebar.removeFromPlayer(player.handle());
